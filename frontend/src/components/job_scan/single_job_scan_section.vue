  ...
  <div class="card">
    ...
    <div class="card-header border-0 pt-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold fs-3 mb-1">ATS Result</span>

        <span class="_text-muted mt-1 fw-semobold fs-7">
          {{ singleScan.ats_analysis?.explanation }}
        </span>
      </h3>
      <!--      <div-->
      <!--        class="card-toolbar"-->
        ...
        >
          ...
          <thead>
            <tr class="fw-bold text-muted">
              ...
              <th class="min-w-140px">Values</th>
              <th class="min-w-140px">Explanations</th>
          ...
          <tbody>
            <template v-for="(item, index) in items" :key="index">
              <tr>
                ...
                <!-- values -->
                <td>
                  <span v-if="item.section.values?.length == 0"> —</span>
                  <ul>
                    <li v-for="(list, i) in item.section.values" :key="i">
                      <i class="fas fa-square-poll-horizontal fs-4"></i>
                      {{ list }}
                    </li>
                  </ul>
                  ...
                  <div class="d-flex flex-column w-100 me-2">
                    <div class="d-flex flex-stack mb-2">
                      <div class="me-2 fs-7 fw-semobold">
                        <span> {{ item.section.suggestion }}</span>
                      </div>
                    </div>
                  </div>
                </td>
